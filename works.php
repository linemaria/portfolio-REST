...
include_once 'classes/Work.class.php';
...
switch($method) {
    case 'GET':
        ...
        $response = $works->getWorks();
        if(count($response) == 0) {
            $response = array("message" => "Inga arbeten hittades!");
        }

        // if(isset($id)) {
        //     //kör funktion för att läsa rad med specifikt id
        //     $response = $works->getWorkById($id);
        // } else {
...
if($data->workplace == "" || $data->title == "" || $data->start == "" || $data->stop == "") {
$response = array("message" => "Fyll i något!");

http_response_code(400); //user error
} else {
    if($works->createWork($data->workplace, $data->title, $data->start, $data->stop)) {
            ...
            case 'DELETE':
                ...
                if(!isset($id)) {
                    ...
                    if($works->deleteWork($id)) {
                        http_response_code(200); //ok
                        $response = array("message" => "Arbetet har raderats.");
                    } else {
                        http_response_code(503); //server error
                        $response = array("message" => "Arbetet har inte raderats.");
                    }
                }
                break;
}
